...
public class ProductProcess extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(process.equals("update")) {
			...
			//商品ID
			int products_id = 0;
			if ((request.getParameter("products_id")!=null)){
				products_id = Integer.parseInt(request.getParameter("products_id"));
			}
			//商品名
			...
			//Beanインスタンスに値を代入する
			products.setProducts_id(products_id);
			...
			products.setUpdate_person(update_person);
			...
			if(photo_check != 0) {
				//実際に画像を格納するパス
				String dirPath= "/Applications/Eclipse_2019-12.app/Contents/workspace/Awamiyage/Awamiyage_PostgreSQL_System/WebContent/img/img_products/" + products_id;
				//img表示用のパス
				String imgPath= "./img/img_products/" + products_id;

				//フォルダ作成
				File folder = new File(dirPath);
				//フォルダ存在確認
				File exists = new File(String.valueOf(products_id));
		        ...
		        //ファイル名の設定
		        String new_name1 = products_id + "-1" + extension1;
		        //指定パスへ画像の書き出し
...
//			        System.out.println(products_eyecatch);
		        ...
		        if(photo_check == 2 || photo_check == 3) {
			        ...
			        String new_name2 = products_id + "-2" + extension2;
			        ...
			        if(photo_check == 3) {
				        ...
				        String new_name3 = products_id + "-3" + extension3;
			...
			//Beanインスタンスに値を代入する
			products.setProducts_id(products_id);
			products.setDelete_time(delete_time);
			products.setDelete_person(delete_person);

			//データベースに商品を挿入する
			ProductsDAO dao = new ProductsDAO();
			count = dao.delete(products);

			//画面に出力
			if (count == 1) {
				request.setAttribute("message", "商品情報を削除しました");
			}else {
				request.setAttribute("message", "商品情報の削除に失敗しました");
			}
		}else {
